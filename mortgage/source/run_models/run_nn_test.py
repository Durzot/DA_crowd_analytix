...
if not os.path.exists(path_model):
    raise ValueError("There is no model %s" % path_model)
...
for index_split in range(mortgage_data.n_splits):
    if opt.cuda:
        network.load_state_dict(torch.load(path_model))  
        network.cuda()
    else:
        network.load_state_dict(torch.load(path_model, map_location='cpu'))
    print("Weights from %s loaded" % path_model)

    # On index_split for which the model was trained just predict on test split and test
    if index_split == opt.index_split:
        mortgage_data = MortgageData(random_state=opt.random_state, other_lim=0.01)
        mortgage_data = mortgage_data.resample(index=index_split)
        ...
        output_test = np.concatenate(list_output, axis=0)
        y_pred_test = np.concatenate(list_y_pred, axis=0)
        ...
        submission_file.to_csv(os.path.join(path_pred, "%s_submission.csv" % opt.model_name), index=False)

    # On other splits retrain the model for a few epochs and predict on test split and test
    else:
        mortgage_data = MortgageData(random_state=opt.random_state, other_lim=0.01)
        mortgage_data = mortgage_data.resample(index=index_split)
        print("Class imbalance corrected on train index")
        ...
        for epoch in range(opt.n_epoch):
            ...
            print("Saving net")
            torch.save(network.state_dict(), os.path.join(save_path, '%s_%s.pth' % (opt.model_name, index_split)))
    ...
    with open(log_file_split, 'a') as log:
        log.write("\n\nSplit [%d/%d]\n" % (index_split+1, mortgage_data.n_splits))
    ...
    idx_train, idx_test = mortgage_data.splits[index_split]
    df_pred_test = pd.DataFrame(np.concatenate((idx_test.reshape(-1,1), pred.reshape(-1,1),
                                                output), axis=1))
    df_pred_test.columns = ["Index test", "Prediction", "Output 0", "Output 1"]
    df_pred_test.to_csv(os.path.join(path_pred, "%s_split_%d.csv" % (opt.model_name, index_split+1)))
    print("Finished!\n")
